...
EOF
    ...
    echo "The API needs to know which org files to include in your agenda."
    echo -e "${YELLOW}Your org files will be at /data/org inside the container.${NC}"
    echo ""
    echo "Configuration options:"
    echo ""
    echo "  1) ${BOLD}Simple setup${NC} - Quick config with one main org file"
    echo "     Good for getting started quickly"
    echo ""
    echo "  2) ${BOLD}Custom setup${NC} - Open editor to configure everything"
    echo "     Full control over agenda files, TODO keywords, capture templates"
    echo ""
    read -p "Choice [1]: " CONFIG_CHOICE
    CONFIG_CHOICE=${CONFIG_CHOICE:-1}

    if [[ "$CONFIG_CHOICE" == "1" ]]; then
        # Simple setup - ask for main org file
        echo ""
        echo "What is the name of your main org file in your repository?"
        echo "(This file should exist at the root of your org repo)"
        echo ""
        echo "Examples: todo.org, tasks.org, inbox.org, main.org"
        echo ""
        read -p "Main org file [todo.org]: " MAIN_ORG_FILE
        MAIN_ORG_FILE=${MAIN_ORG_FILE:-todo.org}

        # Strip .org extension if provided, then add it back
        MAIN_ORG_FILE="${MAIN_ORG_FILE%.org}.org"

        echo ""
        echo "Where should new TODOs be captured? (for the /create-todo API)"
        echo "This can be the same file or a separate inbox file."
        echo ""
        read -p "Capture file [${MAIN_ORG_FILE}]: " CAPTURE_FILE
        CAPTURE_FILE=${CAPTURE_FILE:-$MAIN_ORG_FILE}
        CAPTURE_FILE="${CAPTURE_FILE%.org}.org"

        print_step "Creating simple configuration..."

        cat > custom-config.el << EOF
...
;; Main agenda file
(setq org-agenda-files '("/data/org/${MAIN_ORG_FILE}"))

;; To include more files, add them to the list:
;; (setq org-agenda-files '("/data/org/${MAIN_ORG_FILE}" "/data/org/work.org"))
;;
;; Or include all .org files:
...
;; Capture template for /create-todo API endpoint
(setq org-capture-templates
      '(("t" "Todo" entry (file "/data/org/${CAPTURE_FILE}")
         "* TODO %?\n%U\n")))
...
EOF
        ...
        print_success "Created custom-config.el"
        echo ""
        echo "Configuration summary:"
        echo "  Agenda file:  /data/org/${MAIN_ORG_FILE}"
        echo "  Capture file: /data/org/${CAPTURE_FILE}"
        echo ""
        echo -e "${YELLOW}Make sure '${MAIN_ORG_FILE}' exists in your org repository!${NC}"

    else
        # Custom setup - open editor
        echo ""
        echo -e "${BOLD}Opening custom-config.el for editing...${NC}"
        echo ""
        echo "Key settings to configure:"
        echo ""
        echo "  ${BOLD}org-agenda-files${NC} - which .org files to include"
        echo "    Example: '(\"/data/org/todo.org\" \"/data/org/work.org\")"
        echo ""
        echo "  ${BOLD}org-todo-keywords${NC} - your TODO states"
        echo "    Example: '((sequence \"TODO\" \"DONE\"))"
        echo ""
        echo "  ${BOLD}org-capture-templates${NC} - for /create-todo endpoint"
        echo "    Defines where new TODOs are saved"
        echo ""
        read -p "Press Enter to open ${EDITOR:-nano}..."
